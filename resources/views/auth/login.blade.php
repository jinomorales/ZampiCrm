...
<div class="card-body">
    <div>
        <h2 class="mb-3 f-w-600">{{ __('Ingreso') }}</h2>
    </div>
    @if (session('status'))
    <div>
        <div class="mb-4 font-medium text-lg text-green-600 text-danger">
            {{ __('Su cuenta está deshabilitada, comuníquese con su administrador') }}
        </div>
    </div>
    @endif
    <div class="custom-login-form">
        <form method="POST" action="{{ route('login') }}" class="needs-validation" novalidate="">
            ...
            <div class="form-group mb-3">
                ...
                <input id="email" type="email" class="form-control  @error('email') is-invalid @enderror" name="email" placeholder="{{ __('Ingresar correo') }}" required autofocus>
                @error('email')
        ...
        <p class="my-4 text-center">{{ __("¿No tienes una cuenta?") }}
            <a href="{{ route('register', $lang) }}" tabindex="0">{{ __('Registrarse') }}</a>
        </p>
        @endif
    </div>
</div>
