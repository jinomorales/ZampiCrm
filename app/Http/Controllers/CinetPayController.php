...
class CinetPayController extends Controller
{
        ...
        if ($plan) {
            ...
            try {
                ...
                if (
                    ...
                    return redirect()->route('plan.index')->with('error', __('Availabe currencies: XOF, CDF, USD, KMF, GNF'));
                }
    ...
    public function invoiceCinetPayReturn(Request $request)
    {
        ...
        if (isset($request->transaction_id) || isset($request->token)) {
            $payment_setting = Utility::invoice_payment_settings();
            ...
            $invoice = Invoice::find($cinetpaySession['invoice_id']);
            $get_amount = $cinetpaySession['amount'];
            $currency = isset($payment_setting['currency']) ? $payment_setting['currency'] : '';
            $orderID = strtoupper(str_replace('.', '', uniqid('', true)));
                ...
                try {
                    ...
                    if ($response_body['code'] == '00') {
                        ...
                        try {
                            ...
                            $new->date = Date('Y-m-d');
                            $new->amount = $get_amount ? $get_amount : 0;
                            $new->client_id = $user->id;
                            $new->payment_type = 'CinetPay';
                            $new->save();
                            ...
                            if (Auth::check()) {
                                ...
                                return redirect()->back()->with('error', $response_body['status_message']);
                            }
    ...
    public function invoiceGetcinetpayStatus($data)
    {
        $curl = curl_init();
